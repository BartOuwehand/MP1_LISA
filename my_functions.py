...
def GenerateGalbins(orbit_path,gw_path, fs, size, Amp_true, f_true, phi0_true, gw_beta_true, gw_lambda_true, t0,iota):
    ...
    for a,f,p,beta,lamb in zip(Amp_true, f_true, phi0_true, gw_beta_true, gw_lambda_true):
        source = GalacticBinary(A=a, f=f, phi0=p, orbits=orbit_path ,t0=t0, gw_beta=beta, gw_lambda=lamb, iota=iota, dt=1/fs, size=size+300)
...
def BuildModelTDI(orbit_path,fs,size,Amp_true, f_true, phi0_true, gw_beta_true, gw_lambda_true,iota,discard,detailed=True,interpolate=True):
    ...
    if detailed:
        ...
        for a, f, p, beta, lamb in zip(Amp_true, f_true, phi0_true, gw_beta_true, gw_lambda_true):
            GalBin = GalacticBinary(A=a/f, f=f, phi0=p, orbits=orbit_path, t0=orbits_t0+10, gw_beta=beta, gw_lambda=lamb, iota=iota, dt=1/fs, size=size+300)
        ...
        GalBin = GalacticBinary(A=1e-20, f=1e-3, phi0=0, orbits=orbit_path, t0=orbits_t0+10, gw_beta=0, gw_lambda=0, iota=iota, dt=1/fs, size=size+300)
        GalBin.write('gw_tmp.h5')
        # rawdata = Data.from_gws('gw_tmp.h5',orbit_path,interpolate=interpolate)#, skipped=-int(size))
        rawdata = Data.from_gws('gw_tmp.h5',orbit_path,interpolate=False)#, skipped=-int(size))
        os.remove('gw_tmp.h5')
